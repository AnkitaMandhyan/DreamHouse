# -*- coding: utf-8 -*-
"""
Created on Tue Oct 12 10:26:20 2021
...
st.image('House.jfif')
st.video('https://www.youtube.com/watch?v=d_leYuPu98I')
df=pd.read_csv('kc_house_data.csv')
price_set=st.slider("Price Range",min_value=int(df['price'].min()),max_value=int(df['price'].max()),step=50,value=int(df['price'].min()))
...
for i in df_new.keys():
    try:
        val=st.text_input("Enter no.: Value of "+i)
